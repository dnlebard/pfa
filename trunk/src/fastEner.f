...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc	  
     ...
     x   + dStrIOType + new(" FOR READING FILES")     
        ...
        write(*,*) " ENTER THE NUMBER OF TRAJECTORY CONFIGS: "
        read(*,*) nConf
...
c        write(*,*) tmpStr%data(1:tmpStr%length)    
     ...
     x            " NAME(S) |",trim(strSelect), "| "
        write(*,*)""
        endif     

        if(doCluster) then
        write(*,*) " ENTER THE CLUSTER RESIDUE(S) NAME(S): "
        read(*,'(a)') strResSel
        write(*,*) " USING THE FOLLOWING RESIDUE(S) FOR CLUSTER",
     ...
     x               rBindCut, "|"
        ...
        if(doHoleV) then
        write(*,*) " ENTER THE HOLE PROBE RADIUS (in Ang): "
        read(*,*) holeRadius
        write(*,*) " USING THE HOLE PROBE RADIUS |",
     x               holeRadius, "|"
        write(*,*)""
        write(*,*) " ENTER THE HOLE C.O.M. FILE NAME: "
        read(*,'(a)') strHoleCOM
        write(*,*) " USING THE HOLE C.O.M. FILE NAMED ",
     x            " |",trim(strHoleCOM), "| "
        write(*,*)""
        endif 

        if(doCluster) then
        write(*,*) " ENTER THE CLUSTER CUTOFF LENGTH (in Ang): "
        read(*,*) rClusterCut
        write(*,*) " USING THE CLUSTER CUTOFF LENGTH |",
     x               rClusterCut, "|"
        write(*,*)""
        write(*,*) " ENTER THE NAME OF THE CLUSTER DATA FILE: "
        read(*,'(a)') strClusterOut
        write(*,*) " WRITING CLUSTER DATA TO |",
     x               trim(strClusterOut), "|"
        write(*,*)""
...
ccccc REMOVED THIS BUT MAY NEED IT AGAIN cccccc
        ...
        if (doPSFRead) then                 ! Setup the psf.conf data on all nodes
          call setStartEnd()
        endif
...
!       ++++++++++ end read section +++++++++   !
         ...
         if(doHoleV) call calcHoleV()           ! Calculate the total volume of a cavity via MC
         if(doEFS) call calcEFSHist()           ! Calculate the E-field histogram of the shell
         if(doCluster) call findClusters()      ! Find all aggregates for cluster analysis
...
!!!!!!!!!!!!!!!!
     ...
     x      .and. doCoul) then
         close(iEne)
        elseif((.not. doAllEner) 
     x         .and. doInduced) then
         close(iInd)               
        ...
        if (doHoleV) then ! close the hole volume file
          close(iHoleVol)
        endif
        ...
        if(doCluster) then
         close(iClusterOut)    ! Close the cluster analysis output file
        endif
        if(doGZRead) then
